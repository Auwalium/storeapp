...
<body>
	...
	<div class="container1">
        ...
        <th>RECOMM QTY BY H of Nursing</th>
        <th>edit</th>
        </tr>
        </thead>
        <tbody>
        ...
        $stmt = $db_con->prepare("SELECT stock.slevel, requestitem.name,requestitem.rqa,requestitem.rqhn, requestitem.rdate,requestitem.id, requestitem.dept,requestitem.unit , requestitem.sunit, requestitem.quantity, requestitem.noti,requestitem.commentm,requestitem.comments, requestitem.category 
	FROM stock inner join requestitem where  stock.name = requestitem.name AND (requestitem.noti = 1 AND requestitem.unit = 'nursing')");
        $stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			?>
