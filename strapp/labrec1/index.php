...
<div class="well" style="display: table;">
	...
	<div class="container">
        ...
        $stmt = $db_con->prepare("SELECT * FROM stock where category = 'laboratory' ORDER BY id DESC");
        $stmt->execute();
...
<th width="20">Stock Level</th>           
		   <th width="20">ID</th>
            <th width="40">Name</th>
            <th width="30">Revolving Fund</th>
            <th width="30">Sub Unit</th>
			 <th width="30">Unit</th>
			  <th width="30">Quantity</th>
            <th width="30">RECOMMENDED QTY BY HEAD of LAB</th>
        ...
        $stmt = $db_con->prepare("SELECT stock.code, stock.slevel, requestitem.name, requestitem.rdate,requestitem.id, 
		requestitem.dept,requestitem.unit , requestitem.sunit, requestitem.quantity, requestitem.rqmd1,
		requestitem.commentm,requestitem.comments, requestitem.category FROM stock inner join requestitem 
		where requestitem.name = stock.name AND (stock.category = 'laboratory')");
        $stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			?>
