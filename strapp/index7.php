...
<table width="25%" align = center border="5">
 <tr> 
			...
			<tr> 
				<td>Dispatch By</td>
				<td><input type="int" name="rqmd1"   value=<?php echo "$dispatchby" ?>  ></td>
			</tr>
			...
			<tr> 
				...
				<td><input type="text" id="glass_fee"     name="glass_fee"  value=<?php echo "$rqmd1" ?>  ></td>
...
$result = mysqli_query($mysqli, "SELECT * FROM stock1  ORDER BY id DESC"); // using mysqli_query instead
?>
...
<table cellspacing="0" width="80%" id="example" class="table table-striped table-hover table-responsive">
	<tr bgcolor='#CCCCCC'>
		<td>Name</td>
		<td>Stock Level</td>
		<td>Supplied by</td>
...
//while($res = mysql_fetch_array($result)) { // mysql_fetch_array is deprecated, we need to use mysqli_fetch_array 
	while($res = mysqli_fetch_array($result)) { 		
		...
		echo "<tr>";
		echo "<td>".$res['name']."</td>";
		echo "<td>".$res['slevel']."</td>";
		echo "<td>".$res['sname']."</td>";	
		
		
		echo "<td><a href=\"edit.php?id=$res[id]\">Approved</a> |<a href=\"edit.php?id=$res[id]\">Edit</a> | <a href=\"delete.php?id=$res[id]\" onClick=\"return confirm('Are you sure you want to delete?')\">Delete</a></td>";		
	}
	?>
	</table>
</body>
