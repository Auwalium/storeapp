...
<div class="well" style="display: table;">
    ...
    <div class="container">
        ...
        <!--<button class="btn btn-info" type="button" id="btn-view"> <span class="glyphicon glyphicon-eye-open"></span> &nbsp; View Employee</button>-->
		 ...
		 <hr />
        <a  href="../trace/index.php" title="Click here" target = "new window">
		<img src="edit.png" width="20px" /></span> Click Here to Trace Recieved stock and Dispatch information</button> </a>
		<hr />
        ...
        $stmt = $db_con->prepare("SELECT * FROM stock where category = 'fuel'or category = 'diseal' ");
        $stmt->execute();
...
<body>
 ...
 <th><h1 style="text-decoration:underline"><center><button class="btn btn-info" type="button" id="btn-add"> <span class="glyphicon glyphicon-pencil"></span>Delete Approved Items</th>
        ...
        $stmt = $db_con->prepare("SELECT stock.code, stock.slevel, requestitem.name, requestitem.rdate,requestitem.id, requestitem.dept,requestitem.unit , requestitem.sunit, requestitem.quantity, requestitem.rqa,requestitem.commentm,requestitem.comments, requestitem.category 
	FROM stock inner join requestitem where  stock.name = requestitem.name AND (stock.category = 'fuel' ) ");
        $stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			?>
