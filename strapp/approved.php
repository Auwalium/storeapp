...
<body>
	...
	<div class="container">
        ...
        <h2 class="form-signin-heading">Stock in Store and their Stock Level.</h2><hr />
        ...
        $stmt = $db_con->prepare("SELECT * FROM stock1 ORDER BY code DESC");
        $stmt->execute();
       ...
       $consult_fee=$row['slevel'];
		...
		$qis=$row['qis'];
			...
			<td><?php echo $row['slevel']; ?></td>
						...
						<td> "<td><a href=\"edit.php?id=$res[id]\">Edit</a> | <a href=\"delete.php?id=$res[id]\" onClick=\"return confirm('Are you sure you want to delete?')\">Delete</a></td>";
...
$(document).ready(function() {
	...
	<div class="container">
        ...
        <h2 class="form-signin-heading">Request Items.</h2><hr />
        <hr />
       ...
       <th>delete</th>
        ...
        $stmt = $db_con->prepare("SELECT * FROM requestitem ORDER BY code DESC");
        $stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
		...
		$qis=$row['qis'];
			...
			<td align="center"><a id="<?php echo $row['code']; ?>" class="delete-link" href="#" title="Delete">
