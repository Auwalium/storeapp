...
<body>
	...
	<div class="container1">
        ...
        <center><h2 class="form-signin-heading">Request from Sub-Unit Awaiting Recommendation for Approval.</center></h2><hr />
        ...
        $stmt = $db_con->prepare("SELECT stock.slevel, requestitem.name, requestitem.rqhn, requestitem.rdate,requestitem.id, requestitem.unit,requestitem.dept , requestitem.sunit, requestitem.quantity, requestitem.noti,requestitem.commentm,requestitem.comments, requestitem.category 
	FROM stock inner join requestitem where  stock.name = requestitem.name AND (requestitem.noti = 1 AND requestitem.unit = 'nursing' AND requestitem.category != 'cittu')");
        $stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			?>
