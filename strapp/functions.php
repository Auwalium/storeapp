...
// connect to database
$db = mysqli_connect('localhost', 'emedion', 'Password@1', 'inventory');
...
function login(){
	...
	if (count($errors) == 0) {
		...
		$query = "SELECT * FROM users WHERE username='$username' AND password='$password' and noti = 0 LIMIT 1";
 ...
 //end 
		...
		if (mysqli_num_rows($results) == 1) { // user found
			...
			}elseif ($logged_in_user['user_type'] == 'research') {

				$_SESSION['user'] = $logged_in_user;
				$_SESSION['success']  = "You are now logged in";
				header('location: mdreport2/index.php');
			}
			elseif ($logged_in_user['user_type'] == 'H_clinical') {

				$_SESSION['user'] = $logged_in_user;
				$_SESSION['success']  = "You are now logged in";
				header('location: clinicalA/index.php');
			}elseif ($logged_in_user['user_type'] == 'user') {

				$_SESSION['user'] = $logged_in_user;
				$_SESSION['success']  = "You are now logged in";
				header('location: userrequest/index.php');
				...
				$_SESSION['user'] = $logged_in_user;
				$_SESSION['success']  = "You are now logged in";
				header('location: cittuadmin.php');
				...
				$_SESSION['user'] = $logged_in_user;
				$_SESSION['success']  = "You are now logged in";
				header('location:nursingA/index.php ');	
