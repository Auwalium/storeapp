<?php
	if (isset($_POST['key'])) {
		...
		if ($_POST['key'] == 'getRowData') {
			...
			$sql = $conn->query("SELECT name, dept, quantity, rqmd1,rdate,adate, sunit, unit FROM requestitem WHERE id='$rowID'");
		...
		if ($_POST['key'] == 'getExistingData') {
			...
			$sql = $conn->query("SELECT stock.slevel, requestitem.name, 
						requestitem.rdate,requestitem.adate,requestitem.id, requestitem.dept,requestitem.unit , requestitem.sunit,
						requestitem.quantity, requestitem.rqmd1,requestitem.commentm,requestitem.comments,requestitem.rqa,requestitem.category, requestitem.rqmd1, requestitem.rqmd, requestitem.rqs,requestitem.rqs, requestitem.rqa 
				...
				while($data = $sql->fetch_array()) {
					$response .= '
						<tr>
							...
							<td id="requestitem_'.$data["id"].'">'.$data["rdate"].'</td>
						
							<td id="requestitem_'.$data["id"].'">'.$data["rqa"].'</td>
								
								<td id="requestitem_'.$data["id"].'">'.$data["rqmd"].'</td>
							<td id="requestitem_'.$data["id"].'">'.$data["rqs"].'</td>
							<td>
								<input type="button" onclick="viewORedit('.$data["id"].', \'edit\')" value="Recommend" class="btn btn-primary">
		...
		if ($_POST['key'] == 'deleteRow') {
			$conn->query("UPDATE requestitem SET noti=0 WHERE id='$rowID'");
		...
		if ($_POST['key'] == 'updateRow') {
			$conn->query("UPDATE requestitem SET name='$name', dept='$dept',rdate='$rdate',adate='$adate', quantity='$quantity',rqmd1='$rqmd1',noti = 0 WHERE id='$rowID'");
			exit('success');
		}
