...
<body>
  <div class="well" style="display: table;">
   ...
   $stmt = $db_con->prepare("SELECT * FROM item WHERE stock_level > 0 and item_category != 3  ORDER BY id DESC");
   $stmt->execute();
...
<?php
       		...
       		$stmt = $db_con->prepare("select * from requestitem where rdate = '$today' and rname ='$user' order by id DESC");
       		//print_r($stmt);
		$stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)) { 		
		?>
...
<?php
       		...
       		$stmt = $db_con->prepare("select * from requestitem where rname ='$user' order by id asc");
		$stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)) { 		
		?>
			<tr>
