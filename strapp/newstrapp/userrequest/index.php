...
<body>
  <div class="well" style="display: table;">
	...
	<?php $user = $_SESSION['username'];?>
            <i  style="color: green; font-size: 16px;">(<?php echo ucfirst($_SESSION['user_type']); ?>)</i> 
            <br>
...
?>
       ...
       //require_once 'dbconnet.php';
        ...
        $stmt = $con->prepare("SELECT * FROM item WHERE stock_level > 0   ORDER BY id DESC");
        $stmt->execute();
		print_r($stmt);
...
<?php
       		...
       		$stmt = $con->prepare("select * from requestitem where rdate = '$today' and rname ='$user' ");
		$stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)) { 		
		?>
			<tr>
...
<?php
       		...
       		$stmt = $con->prepare("select * from requestitem where rname ='$user' ORDER BY id DESC");
		$stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)) { 		
		?>
			<tr>
