...
<?php
       	...
       	$stmt = $db_con->prepare("select * from requestitem where rdate = '$today' and rname ='$user' and requestitem.noti != 7 order by id DESC");
       		print_r($stmt);
		$stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)) { 		
		?>
			<tr>
			<td><?php echo $row['id']; ?></td>
			<td><?php echo $row["name"].' '.$row["brand_name"]; ?></td>
...
<?php
       		...
       		$stmt = $db_con->prepare("select * from requestitem where  rname ='$user' AND noti != 5 and noti != 7 order by id asc");
		$stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)) { 		
		?>
			<tr>
...
?>
 ...
 <br>
    ...
    <div class="container">
	   ...
	   <div >     	
      <table  border="10" cellspacing="0" width="100%" class="table table-striped table-hover table-responsive">
	  <center><h4>DECLINED REQUEST AS AT <?php echo $today;  ?> </center></h4>
	<tr bgcolor='#CCCCCC'>
		<td>ID</td>
		<td>ITEM NAME</td>
		<td>REQUEST DATE</td>
...
<?php
       		...
       		$stmt = $db_con->prepare("select requestitem.id,requestitem.name,requestitem.category,requestitem.quantity,requestitem.rqs,requestitem.rqmd,requestitem.nursing,requestitem.rdate,requestitem.rqs,requestitem.rqmd1,requestitem.approved_qty,requestitem.adate,item.generic_name, item.brand_name from requestitem inner join item where  item.generic_name = requestitem.name and  rname ='$user' AND noti = 5 order by id asc");
		$stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)) { 		
		?>
			<tr>
