...
{
	$id = $_GET['edit_id'];	
	$stmt=$db_con->prepare("SELECT requestitem.name, requestitem.dept,requestitem.id,requestitem.adate ,requestitem.rdate ,  requestitem.unit, requestitem.sunit, requestitem.category, requestitem.quantity, requestitem.rqmd,
	requestitem.rqmd1, requestitem.rqa, requestitem.rqs, requestitem.comments, requestitem.commentm, requestitem.rqhn, stock.code, stock.slevel
	FROM stock inner join requestitem where requestitem.name = stock.name and  requestitem.id = $id");
...
$rname = $_SESSION["username"];     
    ...
    <table class='table table-bordered'>
		...
		<tr>
            ...
            <td><input type='text' name='qa' class='form-control' value='<?php echo $row['rqmd1']; ?>' ></td>
        </tr>
		<tr>
            <td>Approved Qty by store</td>
            <td><input type='text' name='qabs' class='form-control' required></td>
        </tr>
        <tr>
		...
		<tr>
		 ...
		 <tr>
            <td>Request Date</td>
            <td><input type='text' name='adate' class='form-control' value='<?php echo $row['rdate']; ?>' ></td>
        </tr>
		 <tr>
            <td>code</td>
            <td><input type='text' name='code' class='form-control'value='<?php echo $row['code']; ?>' required></td>
        </tr>
 
        <tr>
            <td>stock level</td>
            <td><b><input type='text' name='slevel' class='form-control' style="color: red" value='<?php echo $row['slevel']; ?>'  ></b></td> 
