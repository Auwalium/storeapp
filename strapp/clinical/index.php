...
<body>
	...
	<div class="container1">
		...
		<th>RECOMM QTY BY Head of Clinical</th>
        ...
        $stmt = $db_con->prepare("SELECT stock.slevel, requestitem.name, requestitem.rqhn,requestitem.rdate,requestitem.id, requestitem.dept,requestitem.unit , requestitem.sunit, requestitem.quantity, requestitem.noti,requestitem.commentm,requestitem.comments, requestitem.category, requestitem.rqs 
	FROM stock inner join requestitem where  stock.name = requestitem.name AND (requestitem.noti = 1 AND requestitem.dept = 'clinical'AND requestitem.category != 'cittu')");
        $stmt->execute();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			?>
